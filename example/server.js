...
server.on('connection', function $onConnection(socket) {
    ...
    console.log('child %s: add socket %s', process.pid, id);

    socket.on('close', function() {
        console.log('child %s: remove socket %s', process.pid, id);
...
module.exports = {
    start() {
        ...
        console.log('child %s: start listen on 3999', process.pid);
        server.listen(3999);
    },
    close() {
        return new Promise(function $onCloseHandler(resolve) {
            console.log('child %s: closing server...', process.pid);
            Object.keys(_connections).forEach(function $socRem(key) {
                console.log('child %s: remove: %s', process.pid, key);
