    ...
    init(config = {}) {
        ...
        this.cluster.on('exit', (worker, code, signal) => {
            ...
            if (worker.exitedAfterDisconnect === true) {
                this.logger.info('Oh, it was just voluntary – no need to worry');
            } else if (!this.waitingExit || !this.destroyed) {
                this.logger.info('Ah! we should bring it back to life!');
                this.fork();
            } else {
                this.logger.info('we should be gone', code, signal);
            }
        });
